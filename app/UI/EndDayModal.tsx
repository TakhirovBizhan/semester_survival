...
export const EndDayModal = () => {
    ...
    setTimeout(() => {
      ...
      if (player.currentDay === 5) {
        if (player.academic <= 0) {
          ...
          // Хорошая концовка
          router.push("/endings/goodEnding");
        }
      } else {
        // Переход на следующий день
        // router.push("/day" + nextDay);
        // router.push(`/day${nextDay}` as `/day${number}`);
        // router.push(("/day" + nextDay) as unknown as string);
        router.push(`/day${nextDay}` as never);
  ...
  return (
    <ModalWindow visible={true} onClose={() => setModalType("none")}>
      {!isSleeping ? (
        <View>
          <Text style={{ color: "white", fontFamily: "monospace", fontSize: 18, marginBottom: 12, textAlign: "center" }}>
            📅 Конец дня {player.currentDay}
          </Text>
